...
class Recovery_CoordAtt(nn.Module):
    def __init__(self, inp1, oup1, reduction=16):
        ...
        self.final_conv = SignleConv(inp1,(inp1//2))
...
class ContactPatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.expand = nn.Linear(dim, 2 * dim, bias=False) if dim_scale == 2 else nn.Identity()
        self.norm = norm_layer(dim // dim_scale)
        self.recovery_CA = Recovery_CoordAtt(dim,dim)
    ...
    def forward(self, x1,x2):
        ...
        # x1: (B,14,14,4*C)
        x1 = rearrange(x1, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=2, p2=2, c=C // 4)
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if upsample is not None:
            self.upsample = ContactPatchExpand(input_resolution, dim=dim, dim_scale=2, norm_layer=norm_layer)
        else:
            self.upsample = None

    def forward(self, x,x2):
        # x = ContactPatchExpand(x1,x2)
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x)
            else:
...
class SwinTransformerSys(nn.Module):
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         ...
                                         norm_layer=norm_layer,
                                         upsample=ContactPatchExpand if (i_layer < self.num_layers - 1) else None,
                                         use_checkpoint=use_checkpoint)
    ...
    def forward_up_features(self, x, x_downsample):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x,x_downsample[2])
            else:
                # x = torch.cat([x, x_downsample[3 - inx]], -1)
                # x = self.concat_back_dim[inx](x)
                x = layer_up(x,x_downsample[2 - inx])
