...
class Skip_CoordAtt(nn.Module):
    def __init__(self, inp1, oup1, inp2, oup2, inp3, oup3, reduction=16):
        super(Skip_CoordAtt, self).__init__()
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))

        mip = max(8, (inp1+inp2+inp3) // reduction)
        ...
        self.conv = nn.Conv2d(mip,mip, kernel_size=1, stride=1, padding=0)

        self.conv1_h = nn.Conv2d(mip, oup1, kernel_size=1, stride=1, padding=0)
        self.conv1_w = nn.Conv2d(mip, oup1, kernel_size=1, stride=1, padding=0)
        self.conv2_h = nn.Conv2d(mip, oup2, kernel_size=1, stride=1, padding=0)
        self.conv2_w = nn.Conv2d(mip, oup2, kernel_size=1, stride=1, padding=0)
        self.conv3_h = nn.Conv2d(mip, oup3, kernel_size=1, stride=1, padding=0)
        self.conv3_w = nn.Conv2d(mip, oup3, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x1, x2, x3):
        ...
        x3_h = self.pool_h(x3)
        x3_w = self.pool_w(x3).permute(0, 1, 3, 2)

        x1_h = self.conv1(x1_h)
        x1_w = self.conv1(x1_w)
        x2_h = self.conv2(x2_h)
        x2_w = self.conv2(x2_w)
        x3_h = self.conv3(x3_h)
        x3_w = self.conv3(x3_w)

        y = torch.cat([x1_h, x1_w, x2_h, x2_w, x3_h, x3_w], dim=2)
        y = self.conv(y)
        y = self.bn1(y)
        y = self.act(y)
        # x_proj = y
        # y = self.branch_main(y)
        # y = self.acon(y)
        x1_h, x1_w, x2_h, x2_w, x3_h, x3_w = torch.split(y, [h1, w1, h2, w2, h3, w3], dim=2)
        x1_w = x1_w.permute(0, 1, 3, 2)
        x2_w = x2_w.permute(0, 1, 3, 2)
        x3_w = x3_w.permute(0, 1, 3, 2)

        a1_h = self.conv1_h(x1_h).sigmoid()
        a1_w = self.conv1_w(x1_w).sigmoid()
        a2_h = self.conv2_h(x2_h).sigmoid()
        a2_w = self.conv2_w(x2_w).sigmoid()
        a3_h = self.conv3_h(x3_h).sigmoid()
        a3_w = self.conv3_w(x3_w).sigmoid()

        out = []
        out1 = identity_1 * a1_w * a1_h
        out2 = identity_2 * a2_w * a2_h
        out3 = identity_3 * a3_w * a3_h
        out.append(out1)
        out.append(out2)
        out.append(out3)
...
class FinalUp(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        mode = "bilinear"
        self.up = nn.Upsample(scale_factor=4, mode=mode, align_corners=True)
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.dec1 = Up(1152, 384)
        self.dec2 = Up(576, 192)
        self.dec3 = Up(288, 96)
        self.dec4 = FinalUp(96,48)
        self.output1 = nn.Conv2d(in_channels=48, out_channels=self.num_classes, kernel_size=1, bias=False)
